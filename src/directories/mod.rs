...
#[cfg(test)]
mod tests {
    //! Tests for the read surface (`list`) of this module, which exercises
    //! the round-trip through `commands::add` and `commands::soft_remove`
    //! since those are the public way to create/remove directory rows.
    //! Pure mutation tests live in `commands.rs::tests`.
